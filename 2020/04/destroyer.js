/**
 * @desc 一个包含两个数字的数组，计算这两个数字和它们之间所有数字的和。
 * @param { number[] } arr
 * @return { number }
 */
const arrayItemTotal = (arr) => {
...
const arr = [5, 10];
console.log(arrayItemTotal(arr));

/**
 * @desc 实现一个摧毁(destroyer)函数，第一个参数是待摧毁的数组，其余的参数是待摧毁的值。
 * @param { any[] } arr
 * @return { any[] }
 */
const destroyer = (arr, ...rest) => {
  ...
  for (let i = 0; i < rest.length; i += 1) {
    destroyEleArr.push(rest[i]);
  }

  return arr.filter((arg) => !destroyEleArr.includes(arg));
};

console.log(destroyer([3, 5, 7, 9, 'Willa'], 'Willa', 9));
